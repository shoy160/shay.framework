...
namespace Shay.Dapper
{
    ...
    public class SQL
    {
        ...
        /// <returns></returns>
        public void Paged(int page, int size)
        {
            if (!IsSelect())
                return;
            var sql = ToString();
            ...
            {
                throw new Exception("need order by!");
            }

            sql = sql.Replace($" {order}", string.Empty);
            sql = sql.Replace(columns, $"ROW_NUMBER() OVER({order}) AS [paged_row],{columns}");

            var countSql = _sqlBuilder.ToString().Replace(columns, "COUNT(1) ").Replace($" {order}", string.Empty);
            sql =
