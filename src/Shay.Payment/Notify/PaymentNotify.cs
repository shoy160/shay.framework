...
namespace Shay.Payment.Notify
{
    public class PaymentNotify
    {
        ...
        /// </summary>
        /// <returns></returns>
        private static bool IsXmlData => HttpContext.ContentType == "text/xml" || HttpContext.ContentType == "application/xml";

        /// <summary>
        /// 是否是GET请求
        /// </summary>
        /// <returns></returns>
        private static bool IsGetRequest => HttpContext.RequestType == "GET";
        ...
        public static GatewayData ReadNotifyData()
        {
            ...
            if (IsGetRequest)
            {
                gatewayData.FromUrl(HttpContext.QueryString);
            }
            else
            {
                if (IsXmlData)
                {
                    var reader = new StreamReader(HttpContext.Body);
                ...
                else
                {
                    try
                    {
                        gatewayData.FromForm(HttpContext.Form);
                    }
                    catch { }
                }
            }
